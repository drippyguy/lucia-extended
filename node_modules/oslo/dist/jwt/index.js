...
exports.parseJWT = exports.validateJWT = exports.createJWT = void 0;
...
async function createJWT(algorithm, key, payloadClaims, options) {
    ...
    const headerPart = index_js_2.base64url.encode(textEncoder.encode(JSON.stringify(header)), {
        includePadding: false
    });
    const payloadPart = index_js_2.base64url.encode(textEncoder.encode(JSON.stringify(payload)), {
        includePadding: false
    });
    const data = textEncoder.encode([headerPart, payloadPart].join("."));
    const signature = await getAlgorithm(algorithm).sign(key, data);
    const signaturePart = index_js_2.base64url.encode(new Uint8Array(signature), {
        includePadding: false
    });
...
async function validateJWT(algorithm, key, jwt) {
    ...
    if (parsedJWT.expiresAt && !(0, index_js_3.isWithinExpirationDate)(parsedJWT.expiresAt)) {
        throw new Error("Expired JWT");
    }
    if (parsedJWT.notBefore && Date.now() < parsedJWT.notBefore.getTime()) {
        throw new Error("Inactive JWT");
    }
    const signature = index_js_2.base64url.decode(parsedJWT.parts[2], {
        strict: false
    });
...
function parseJWT(jwt) {
    ...
    const rawHeader = index_js_2.base64url.decode(jwtParts[0], {
        strict: false
    });
    const rawPayload = index_js_2.base64url.decode(jwtParts[1], {
        strict: false
    });
...
function getAlgorithm(algorithm) {
    if (algorithm === "ES256" || algorithm === "ES384" || algorithm === "ES512") {
        return new index_js_1.ECDSA(ecdsaDictionary[algorithm].hash, ecdsaDictionary[algorithm].curve);
    }
    if (algorithm === "HS256" || algorithm === "HS384" || algorithm === "HS512") {
        return new index_js_1.HMAC(hmacDictionary[algorithm]);
    }
    if (algorithm === "RS256" || algorithm === "RS384" || algorithm === "RS512") {
        return new index_js_1.RSASSAPKCS1v1_5(rsassapkcs1v1_5Dictionary[algorithm]);
    }
    if (algorithm === "PS256" || algorithm === "PS384" || algorithm === "PS512") {
        return new index_js_1.RSASSAPSS(rsassapssDictionary[algorithm]);
    }
